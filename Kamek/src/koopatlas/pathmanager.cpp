...
void dWMPathManager_c::unlockPaths() {
	...
	for (int i = 0; i < pathLayer->pathCount; i++) {
		...
		path->setLayerAlpha((path->isAvailable == dKPPath_s::ALWAYS_AVAILABLE) ? 255 : 99);
	}
	...
	while (*in != 0) {
		...
		for (int i = 0; i < affectedCount; i++) {
			...
			path->setLayerAlpha(value ? 255 : 99);
	...
	for (int i = 0; i < pathLayer->nodeCount; i++) {
		...
		node->setLayerAlpha((node->isUnlocked() & !node->isNew) ? 255 : 99); //0 not 99
	}
}
...
bool dWMPathManager_c::doingThings() {
	if (isEnteringLevel || (waitAfterUnlock > 0) || (completionAnimDelay > 0) ||
			...
			(countdownToFadeIn > 0) || (unlockingAlpha != 99-1) ||
...
void dWMPathManager_c::execute() {
	...
	if (panningCameraToPaths) {
		...
		unlockingAlpha = 99; //0 not 99 
	}

	if (unlockingAlpha != 99-1) {
		unlockingAlpha += 3;
		...
		if (unlockingAlpha == 255) {
			...
			for (int i = 0; i < pathLayer->nodeCount; i++) {
				...
				if (node->isNew && node->type == dKPNode_s::LEVEL) {
					...
					Vec efScale2 = {1.8f,1.8f,1.8f};
					if (node->levelNumber[1] == 15 || node->levelNumber[1] == 25) {//tower/castle 
						SpawnEffect("Wm_cs_pointlight", 0, &efPos, &efRot, &efScale2); 
					} else {
	...
	if (isMoving) {
		...
		//	dWorldCamera_c::instance->panToPosition(dWorldCamera_c::instance->currentX, dWorldCamera_c::instance->currentY,STD_ZOOM-0.7);
			if (currentNode->levelNumber[1] != 97 && currentNode->levelNumber[1] != 96 && currentNode->levelNumber[1] != 95 && currentNode->levelNumber[1] != 94) {
...
void dWMPathManager_c::startMovementTo(dKPPath_s *path) {
	...
	static const struct {
		...
		{run,2.0f,10.0f, -1,-1.0f, SE_PLY_FOOTNOTE_CS_DIRT,SE_NULL,0,0}, //walk water (FAKE) 
	...
	is_fake_walk_water = (path->animation == dKPPath_s::WALK_WATER); //walk water will be replaced 
...
void dWMPathManager_c::moveThroughPath(int pressedDir) {
	...
	if (is_fake_walk_water == true) {
		dWorldCamera_c::instance->zoomLevel = STD_ZOOM + 1.0;

	} else {
		dWorldCamera_c::instance->zoomLevel = STD_ZOOM;	
	}
...
void dWMPathManager_c::activatePoint() {
	...
	if (currentNode->type == dKPNode_s::LEVEL) {
		...
		if (l != 99-1) {
			dWorldCamera_c::instance->panToPosition(dWorldCamera_c::instance->currentX, dWorldCamera_c::instance->currentY, 1.1);
		}
		...
		if (l == 25-1 || l == 15-1) {
			PlaySoundWithFunctionB4(SoundRelatedClass, &something2, (Player_Powerup[0] == 3) ? SE_VOC_MA_PLAYER_DECIDE_MAME: SE_VOC_MA_CLEAR_LAST_BOSS, 1);
