...
int dWMMap_c::onCreate() {
	...
	showLaunchStar2 = false;
	showLaunchStar3 = false;
	showLaunchStar4 = false;
	...
	//mandy I guess's code
	//dKPMapData_c *dataCls = &dScKoopatlas_c::instance->mapData;
   //for (int i = 0; i < dataCls->pathLayer->nodeCount; i++) {
   //    dKPNode_s *node = dataCls->pathLayer->nodes[i];
   //    if (node->type == dKPNode_s::LEVEL && node->levelNumber[1] == 98) {
	//		
   //        launchStarX = node->x;
   //        launchStarY = node->y;
   //        showLaunchStar = true;
...
//
	//	nw4r::g3d::ResAnmChr lsAnm = lsRes.GetResAnmChr("walk");
...
//fix this   
	...
	this->pathgoomba->enemy_active = true; 
...
//
//
//	//	OSReport("GOOMBA EXECUTE \n");
//		OSReport("posgoomba %x, %s", launchStarX,launchStarX); 
//
//		OSReport("posgoomba %x, %s", launchStarY,launchStarY); 
//	}
//	

		
	
//	if (this->pathgoomba->cangoom == false) {
//		if (this->pathgoomba->timer2 == 0) {
//			spinLaunchStar();
//			this->pathgoomba->timer2 = 2;
//
//		}
//		//anmChr2 = this->resFile2.GetResAnmChr("damage");
//		//this->chrAnimation2.bind(&this->bodyModel2, anmChr2, 1);
//		//this->bodyModel2.bindAnim(&this->chrAnimation2, 0.0);
//		//this->chrAnimation2.setUpdateRate(1.0);
//		
//
//
//	}
...
void dWMMap_c::renderPathLayer(dKPLayer_s *layer) {
	...
	for (int i = 0; i < layer->nodeCount; i++) {
		...
		if (node->type == dKPNode_s::LEVEL) {
			...
			short rz = 0x6000; //96 
			short rx = 0x4000; //176.4
			short ry = 0x8000; //248


			

			node->extra->matrix.translation(node->x, -node->y + 4.0, 1000.0); //originally 498.0
...
void dWMMap_c::spinLaunchStar() {
	nw4r::g3d::ResFile lsRes(getResource("StarRing", "g3d/StarRing.brres"));
	nw4r::g3d::ResAnmChr lsAnm = lsRes.GetResAnmChr("damage");
	launchStarAnm.bind(&launchStarModel, lsAnm, false);
	launchStarModel.bindAnim(&launchStarAnm); 
}
